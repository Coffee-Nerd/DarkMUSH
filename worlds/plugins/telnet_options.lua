...
local function TelnetOption(which, on)
  ...
  if on then
    SendPkt(string.char(IAC, SB, DarkMUSH_TELOPT, which, TELOPT_ON, IAC, SE))
  else
    SendPkt(string.char(IAC, SB, DarkMUSH_TELOPT, which, TELOPT_OFF, IAC, SE))
  end -- if
end   -- TelnetOption
